...
class UniLoRAConfig(PeftConfig):
    """
    This is the configuration class to store the configuration of a [`UniLoRAConfig`].
    ...
    Args:
        ...
        proj_seed (`int`):
            Random seed for initializing the projection matrix.
        ...
        unilora_dropout (`float`):
            The dropout probability for UniLoRA layers.
        fan_in_fan_out (`bool`):
            Set this to True if the layer to replace stores weight like (fan_in, fan_out). For example, gpt-2 uses
            `Conv1D` which stores weights like (fan_in, fan_out) and hence this should be set to `True`.
        bias (`str`):
            Bias type for UniLoRA. Can be 'none', 'all' or 'unilora_only'. If 'all' or 'unilora_only', the corresponding
        ...
        modules_to_save (`List[str]`):
            List of modules apart from UniLoRA layers to be set as trainable and saved in the final checkpoint.
        init_theta_d_bound (`float`):
            The vector bank is initialized with a uniform distribution between -init_theta_d_bound and
            init_theta_d_bound. Avoid initializing the vector bank with all zeros to prevent zero gradients. A
    ...
    bias: str = field(
        default="none",
        metadata={
            "help": "Bias type for UniLoRA. Can be 'none', 'all' or 'unilora_only'"
        },
    )
