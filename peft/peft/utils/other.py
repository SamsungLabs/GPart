        ...
        if adapter_name not in self.modules_to_save:
            with context_manager:
                if tied_module:
                    ...
                    # Check if there's already a trained module from another adapter that we should inherit from
                    # This preserves trained weights during sequential finetuning
                    source_module = self.original_module
                    if self.modules_to_save:
                        # Get the first available trained module to inherit weights from
                        for existing_adapter_name, existing_module in self.modules_to_save.items():
                            if existing_adapter_name != adapter_name:
                                source_module = existing_module
                                break
                    
                    self.modules_to_save[adapter_name] = copy.deepcopy(source_module)
