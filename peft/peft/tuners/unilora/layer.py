...
class UniLoRALayer(BaseTunerLayer):
    ...
    # Note: unilora_theta_d is typically a shared parameter but is referenced in the layer
    ...
    def __init__(self, base_layer: nn.Module, **kwargs):
        ...
        # Changed: Renamed to reflect that these are indices (LongTensor)
        self.unilora_indices_A = BufferDict({}, persistent=True)
        self.unilora_indices_B = BufferDict({}, persistent=True)

        # Changed: Renamed to reflect that these are scaling factors (Float)
        self.unilora_scales_A = BufferDict({}, persistent=True)
        self.unilora_scales_B = BufferDict({}, persistent=True)

        # for privileged basis experiment
        self.mode = "Full"
        self.fixed_A = BufferDict({}, persistent=True)
        self.fixed_B = BufferDict({}, persistent=True)
    ...
    def update_layer(
        ...
        self.unilora_dropout.update(
            nn.ModuleDict({adapter_name: unilora_dropout_layer})
        )

        # Store the reference to the shared parameter bank
    ...
    def reset_unilora_parameters(self, adapter_name, theta_d_length):
        ...
        if adapter_name in self.unilora_theta_d.keys():
            ...
            indices_A = torch.randint(
                0,
                theta_d_length,
                (self.r[adapter_name], self.in_features),
                dtype=torch.long,
            )
            indices_B = torch.randint(
                0,
                theta_d_length,
                (self.out_features, self.r[adapter_name]),
                dtype=torch.long,
            )
            ...
            fixed_A = torch.empty(self.r[adapter_name], self.in_features)
            fixed_B = torch.empty(self.out_features, self.r[adapter_name])
            nn.init.orthogonal_(fixed_A)
            nn.init.orthogonal_(fixed_B)
            self.fixed_A[adapter_name] = fixed_A
            self.fixed_B[adapter_name] = fixed_B
...
class Linear(nn.Linear, UniLoRALayer):
    ...
    def _get_lora_matrices(
        ...
        # Get base layer device and dtype for proper device alignment
        base_layer = self.get_base_layer()
        target_device = base_layer.weight.device
        target_dtype = base_layer.weight.dtype

        # Access indices and move to target device
        unilora_indices_A = self.unilora_indices_A[adapter].to(device=target_device)
        unilora_indices_B = self.unilora_indices_B[adapter].to(device=target_device)
        unilora_scales_A = self.unilora_scales_A[adapter].to(device=target_device, dtype=target_dtype)
        unilora_scales_B = self.unilora_scales_B[adapter].to(device=target_device, dtype=target_dtype)

        # Move theta_d to target device
        unilora_theta_d = self.unilora_theta_d[adapter].to(device=target_device, dtype=target_dtype)

        if cast_to_fp32:
            # Cast to float32 if needed for CPU computation
            unilora_theta_d = unilora_theta_d.float()
            unilora_scales_A = unilora_scales_A.float()
            unilora_scales_B = unilora_scales_B.float()
        ...
        if self.mode == "B_only":
            ...
            B = (
                unilora_theta_d[unilora_indices_B]
                * unilora_scales_B
            )
        elif self.mode == "A_only":
            A = (
                unilora_theta_d[unilora_indices_A]
                * unilora_scales_A
            )
        ...
        # Ensure final matrices are on target device and dtype
        A = A.to(device=target_device, dtype=target_dtype)
        B = B.to(device=target_device, dtype=target_dtype)

        # Cast back if necessary for fp32 operations
        if cast_to_fp32:
            A = A.float()
            B = B.float()
    ...
    def get_delta_weight(self, adapter) -> torch.Tensor:
        ...
        # Ensure indices are on target device for device check
        device = self.unilora_indices_A[adapter].to(device=target_device).device
        dtype = self.unilora_theta_d[adapter].to(device=target_device, dtype=target_dtype).dtype

        cast_to_fp32 = device.type == "cpu" and dtype == torch.float16

        A, B = self._get_lora_matrices(adapter, cast_to_fp32)

        # Ensure final computation is on target device
        A = A.to(device=target_device, dtype=target_dtype)
        B = B.to(device=target_device, dtype=target_dtype)
    ...
    def forward(self, x: torch.Tensor, *args, **kwargs) -> torch.Tensor:
        ...
        # Get base layer device for proper device alignment
        base_layer = self.get_base_layer()
        target_device = base_layer.weight.device
        target_dtype = base_layer.weight.dtype
        
        # Move input to target device and dtype
        ...
        if self.disable_adapters:
            ...
            for active_adapter in self.active_adapters:
                ...
                # Ensure dropout is on correct device and input is proper dtype
                dropout = self.unilora_dropout[active_adapter]
                x_dropout = dropout(x.to(A.dtype))

                # Standard LoRA calculation: x @ A @ B
                # All tensors are now guaranteed to be on the same device
