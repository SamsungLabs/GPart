...
# Assuming you have this mapping, if not you can import or define it like VBLoRA
...
class UniLoRAModel(BaseTuner):
    ...
    # If you have UniLoRA-specific mapping, replace this, otherwise usually reuse LoRA mapping
    target_module_mapping = TRANSFORMERS_MODELS_TO_LORA_TARGET_MODULES_MAPPING

    def __init__(
        self, model, config, adapter_name, low_cpu_mem_usage: bool = False
    ) -> None:
        # BaseTuner's init will call _create_and_replace to inject layers
        super().__init__(
        ...
        # 1. Count total required indices (using new indices variable)
        LoRA_para_cnt = 0
        ...
        # 2. Generate globally uniformly distributed indices
        # Handle both dict and UniLoRAConfig object
        ...
        # 3. Distribute generated indices back to each layer (replace initialized random indices)
        for name, module in model.named_modules():
            if isinstance(module, UniLoRALayer):
        ...
        # 4. Calculate usage frequency of each index for normalization (Scales)
        ...
        # Extract corresponding scale factors based on indices actually used by each layer
        norm_factors = [sqrt_counts[t] for t in index_ls]

        # 5. Update Scales for each layer (original Norm)
        uni_modules = [m for m in self.modules() if isinstance(m, UniLoRALayer)]
        # zip(*[iter(norm_factors)] * 2) will pair the list (scale_A, scale_B)
        for module, (scale_a, scale_b) in zip(
            uni_modules, zip(*[iter(norm_factors)] * 2)
        ):
    ...
    def _init_unilora_theta_d(self, config: UniLoRAConfig, adapter_name: str) -> None:
        ...
        torch.nn.init.uniform_(
            unilora_theta_d, -config.init_theta_d_bound, config.init_theta_d_bound
        )
        self.unilora_theta_d[adapter_name] = unilora_theta_d

    def _pre_injection_hook(
        self, model: nn.Module, config: UniLoRAConfig, adapter_name: str
    ) -> None:
    ...
    def _create_new_module(
        unilora_config, unilora_theta_d, adapter_name, target, **kwargs
    ):
    ...
    def get_nb_savable_parameters(self, adapter="default") -> tuple[int, int]:
        ...
        for name, param in self.named_parameters():
            ...
            # [Modified] Updated to check indices and scales
